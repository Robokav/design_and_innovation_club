...
// Project Card Component (Copied from Projects.jsx for consistency)
...
const ActiveProjects = () => {
  ...
  // This will now crash if the data file is empty or data structure is wrong
  const activeProjectsList = projects.filter(p => p.category === 'active');
    ...
    <section className="py-20 bg-gray-50 min-h-screen pt-32">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16 animate-fade-in-up">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">Current ongoing initiatives where our team is pushing the boundaries of innovation.</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up">
          {activeProjectsList.map((project) => (<ProjectCard key={project.id} project={project} />))}
        </div>
      </div>
    </section>
  );
